...
import {Component, OnInit, Renderer2, ViewChild, ChangeDetectorRef, ElementRef, AfterViewInit, NgZone, HostListener, OnDestroy, ChangeDetectionStrategy} from '@angular/core';
...
import { delay, mergeMap, startWith, takeUntil, takeWhile } from 'rxjs/operators';	
...
class file_region {
  public shape_attributes: {}; // region shape attributes
  public region_attributes?: {}; // region attributes
  constructor() {
...
// Redeclare under the VIAComponent. 
...
export class file_metadata {
  ...
  constructor(filename, size) {
    this.filename = filename; // file size in bytes
    this.size = size;  // array of file_region()
    this.regions = [];  // image attributes
...
export class ViaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('region_info', { static: true }) region_info: ElementRef<HTMLElement>;

	@HostListener('window:keydown', ['$event'])
  keyEvent(event: KeyboardEvent) {
    console.log('KEY DOWN!!!!')
    this._via_handle_global_keydown_event(event);
  }
...
//  region_attributes: {};
  ...
  _via_canvas_regions: file_region[];// image regions spec. in canvas space
  ...
  // Started adding new stuff from here; 
  optionName: any;
  objectTypes: string[] = [
    'Radio',
    'Checkbox',
  ...
  ObjectTags: FormGroup;
  objectTag: FormControl;
  ObjectType: FormControl;
  clickObservable: Observable<Event>;


  current_image: HTMLImageElement;
  Mmousemove: Observable<Event>;  
  Mmousedown: Observable<Event>;
  Mmouseup: Observable<Event>;
  Mmouseout: Observable<Event>;
  Mmouseover: Observable<Event>;
  Ddblclick: Observable<Event>;
  Wwheelevent: Observable<Event>;
  KKeydown: Observable<Event>;
  KKeyup: Observable<Event>;
  propagateNot: any;
  mmouseedown: Subscription;
  mmousemove: Subscription;
  mmouseover: Subscription;
  mmouseup: Subscription;
  ddblclick: Subscription;
  wheelevent: Subscription;
  kkeyup: Subscription;
  kkeydown: Subscription;
  ...
  KeyBoardEventSubscriptions: Subscription;
  ...
  this._via_img_metadata = {}; // image content {abs. path, url, base64 data, etc}
...
//		image canvas
  ...
  this._via_reg_canvas = this.region_canvas.nativeElement;
...
this.VIA_POLYGON_SEGMENT_SUBTENDED_ANGLE = 5;   // in degree (used to approximate shapes using polygon)
  ...
  this.propagateNot = false;





    // let mouseEvents = ['mousemove', 'mousedown', 'mouseup', 'mouseout', 'dblclick', 'WheelEvent', 'mouseover'];
    // this.canvasMouseEventSubscriptions = mouseEvents.map(
    //   (mouseEvent: string) =>
    //     fromEvent(this.region_canvas.nativeElement, mouseEvent)
    //       .subscribe((event: MouseEvent) => this._via_init_mouse_handlers(event))
    // )

    this.Mmousemove = fromEvent(this.region_canvas.nativeElement, 'mousemove');
    this.Mmousedown = fromEvent(this.region_canvas.nativeElement, 'mousedown');
    this.Mmouseup = fromEvent(this.region_canvas.nativeElement, 'mouseup');
    this.Ddblclick = fromEvent(this.region_canvas.nativeElement, 'dblclick');
    this.Mmouseover = fromEvent(this.region_canvas.nativeElement, 'mouseover');
    this.Wwheelevent = fromEvent(this.region_canvas.nativeElement, 'WheelEvent');
    this.KKeyup = fromEvent(this.region_canvas.nativeElement, 'keyup');
    this.KKeydown = fromEvent(this.region_canvas.nativeElement, 'keydown');
  ...
  createFormControls() {
    this.objectTag = new FormControl('');
    this.ObjectType = new FormControl('');
    
  }
  createForm() {
    this.ObjectTags = new FormGroup({
  ...
  subscribeToObservable() {
    this.mmousemove = this.Mmousemove //pipe(takeUntil(this.preserve_aspect_ratio))
      
      .subscribe((e: MouseEvent) => {
      console.log('region_canvas mousemoved!');
      this._via_reg_canvas_mousemove_handler(e);
      
    })

    this.mmouseedown = this.Mmousedown.subscribe((e: MouseEvent) => {
      console.log('region_canvas mouse down!');
      this._via_reg_canvas_mousedown_handler(e);
    });

    this.mmouseover = this.Mmouseover.subscribe((e:MouseEvent) => {
      console.log('region_canvas mouse over!');
      this._via_redraw_reg_canvas(); //VISIT THIS LATER
			this._via_reg_canvas.focus();
    })
    this.mmouseup = this.Mmouseup //.pipe(takeUntil(this.Mmousedown))
      .subscribe((e: MouseEvent) => {
      console.log('region_canvas mouse up!');
      this._via_reg_canvas_mouseup_handler(e);
    })
    this.ddblclick = this.Ddblclick.subscribe((e:MouseEvent) => {
      console.log('region_canvas dbclick!');
      this._via_reg_canvas_dblclick_handler(e);
    })
    this.wheelevent  = this.Wwheelevent.subscribe((e:MouseEvent) => {
      console.log('region_canvas wheelEvent!');
      this._via_reg_canvas_mouse_wheel_listener(e);
    })
    this.kkeydown = this.KKeydown.subscribe((e:KeyboardEvent) => {
      console.log('region_canvas keydown');
      this._via_reg_canvas_keydown_handler(e);
    })
    this.kkeyup = this.KKeyup.subscribe((e:KeyboardEvent) => {
      console.log('region_canvas Keyup!');
      this._via_reg_canvas_keyup_handler(e);
    })
  ...
  // file_metadata(filename: any, size: any) {
  ...
  _via_move_region(region_id: any, move_x: any, move_y: any): void {
    ...
    switch (canvas_attr['name']) {
      ...
      case this.VIA_REGION_SHAPE.POLYGON:
        ...
        // clone for reverting if validation fails
  ...
  _via_redraw_reg_canvas(): void {
    if (this._via_current_image_loaded) {
      ...
      if (this._via_canvas_regions.length > 0) {
        console.log('Found canvas_regions length')
        if (this._via_is_region_boundary_visible) {
          console.log('About to draw region_boundaries')
          this.draw_all_regions();
        }
        if (this._via_is_region_id_visible) {
          console.log('region_id is visible')
  ...
  _via_clear_reg_canvas(): void {
    console.log('Clearing region_canvas!')
    this._via_reg_ctx.clearRect(0, 0, this._via_reg_canvas.width, this._via_reg_canvas.height);
  }
  ...
  _via_draw_polygon_region(all_points_x: any, all_points_y: any, is_selected: any, shape: any): void {
    if (this.is_selected) {
      ...
      if (this.shape === this.VIA_REGION_SHAPE.POLYGON) {
        this._via_reg_ctx.lineTo(all_points_x[0], all_points_y[0]); // close loop
      }
      this._via_reg_ctx.stroke();
      ...
      this._via_reg_ctx.fill();
      this._via_reg_ctx.globalAlpha = 1.0;
      for (i = 0; i < all_points_x.length; ++i) {
        this._via_draw_control_point(all_points_x[i], all_points_y[i]);
      }
    } else {
      // draw a fill line
      this._via_reg_ctx.lineWidth = this.VIA_THEME_REGION_BOUNDARY_WIDTH / 2;
...
//    var region = this._via_img_metadata[this._via_image_id].regions[this.rindex];
  ...
  //
  is_inside_region(px: any, py: any, descending_order: boolean): number {
  ...
  is_inside_polygon(all_points_x: any, all_points_y: any, px: any, py: any): 1 | 0 {
    ...
    var is_left_value = this.is_left(all_points_x[n - 1], all_points_y[n - 1],
  ...
  is_on_region_corner(px: any, py: any): number[] {
     this._via_region_edge = [-1, -1]; // region_id, corner_id [top-left=1,top-right=2,bottom-right=3,bottom-left=4]

    for (var i = 0; i < this._via_canvas_regions.length; ++i) {
      var attr = this._via_canvas_regions[i].shape_attributes;
      this._via_region_edge[0] = i;
  ...
  _via_handle_global_keydown_event(e: Event): void {
    ...
    if ((e as KeyboardEvent).key === 'Escape') {
      ...
      if (this._via_is_user_drawing_polygon) {
        this._via_is_user_drawing_polygon = false;
        this._via_canvas_regions.splice(this._via_current_polygon_region_id, 1);
      }
  ...
  _via_reg_canvas_keyup_handler(e: KeyboardEvent): void {
    if ((e as KeyboardEvent).ctrlKey) {
      this._via_is_ctrl_pressed = false;
      console.log('Ctrl key has been set to False');
    }
  }
  ...
  _via_polyshape_delete_last_vertex(): void {
    if (this._via_is_user_drawing_polygon) {
      var npts = this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_x'].length;
      if (npts > 0) {
        this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_x'].splice(npts - 1, 1);
        this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_y'].splice(npts - 1, 1);
  ...
  _via_polyshape_add_new_polyshape(img_id: any, region_shape: any, region_id: any): void {
    // add all polygon points stored in this._via_canvas_regions[]
  ...
  copy_sel_regions(): void {
    ...
    if (this._via_is_region_selected ||
      ...
      for (var i = 0; i < this._via_img_metadata[this._via_image_id].regions.length; ++i) {
        ...
        if (this._via_region_selected_flag[i]) {
          this._via_copied_image_regions.push(this.clone_image_region(img_region));
        }
      }
  ...
  show_last_image(): void {
    ...
    if (this._via_img_count > 0) {
     ...
     //0 this._via_show_img(this._via_img_fn_list_img_index_list[last_img_index]);
    }
  }
  ...
  set_zoom(zoom_level_index: any): void {
    ...
    var zoom_scale = this.VIA_CANVAS_ZOOM_LEVELS[this._via_canvas_zoom_level_index];
    this.set_all_canvas_scale(zoom_scale);
    var canvas_w = (this._via_current_image_width * zoom_scale) / this._via_canvas_scale_without_zoom;
    var canvas_h = (this._via_current_image_height * zoom_scale) / this._via_canvas_scale_without_zoom;
    this.set_all_canvas_size(canvas_w, canvas_h);
...
//    this._via_canvas_scale = this._via_canvas_scale_without_zoom / zoom_scale;
  ...
  zoom_out(): void {
    ...
    if (this._via_canvas_zoom_level_index === 0) {
      ...
      var new_zoom_level_index = this._via_canvas_zoom_level_index - 1;
      this.set_zoom(new_zoom_level_index);
      this.show_message('Zoomed out to level ' + this.VIA_CANVAS_ZOOM_LEVELS[this._via_canvas_zoom_level_index] + 'X', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
    }
  }
  ...
  toggle_region_info_visibility(): void {
    var elem = this.region_info.nativeElement;
  ...
  img_fn_list_ith_entry_html(i: any): string {
    ...
    htmli += ' (click)="jump_to_image(' + (i) + ')" title="' + this._via_image_filename_list[i] + '">[' + (i + 1) + '] ' + filename + '</li>';
    return htmli;
  }
  ...
  show_attribute_properties(): void {
    var attr_list = document.getElementById('attributes_name_list') as HTMLSelectElement;
  ...
  setup_user_input_panel(ok_handler: any, input: any, config: any, cancel_handler: any): void {
    ...
    html.push('<div class="user_confirm">' +
      '<span class="ok">' +
      '<button id="user_input_ok_button" (click)="user_input_parse_and_invoke_handler()">&nbsp;OK&nbsp;</button></span>' +
      '<span class="cancel">' +
      '<button id="user_input_cancel_button" (click)="user_input_cancel_handler()">CANCEL</button></span></div>');
    ...
    if (this._via_annotation_editor_mode === this.VIA_ANNOTATION_EDITOR_MODE.SINGLE_REGION) {
      ...
      if (this._via_is_region_selected &&
        ...
        if (this._via_settings_ui_image_on_image_annotation_editor_placement === this.VIA_ANNOTATION_EDITOR_PLACEMENT.NEAR_REGION) {
          ...
          // ae.style.top = this.html_position.top;
          // ae.style.left = this.html_position.left;
        }
  ...
  annotation_editor_get_placement(region_id: any): {} {
    ...
    this.html_position.top = this.html_position.top + this._via_img_panel.offsetTop + this.VIA_REGION_EDGE_TOL + 'px';
    this.html_position.left = this.html_position.left + this._via_img_panel.offsetLeft + this.VIA_REGION_EDGE_TOL + 'px';
  ...
  annotation_editor_update_header_html(): void {
  ...
  annotation_editor_extract_html_id_components(html_id: any): {attr_id: number, row_id:number, option_id:number} {
  ...
  annotation_editor_on_metadata_update(p: any): void {
    var pid = this.annotation_editor_extract_html_id_components(p.id);
    var img_id = this._via_image_id;

    var img_index_list = [this._via_image_index];
    var region_id = pid.row_id;
    if (this._via_display_area_content_name === this.VIA_DISPLAY_AREA_CONTENT_NAME.IMAGE_GRID) {
    ...
    if (this._via_metadata_being_updated === 'file') {
      this.annotation_editor_update_file_metadata(img_index_list, pid.attr_id, p.value, p.checked).then(function (update_count) {
        this.annotation_editor_on_metadata_update_done('file', pid.attr_id, update_count);
      }, function (err) {
        console.log(err)
    ...
    if (this._via_metadata_being_updated === 'region') {
      this.annotation_editor_update_region_metadata(img_index_list, region_id, pid.attr_id, p.value, p.checked).then(function (update_count) {
        this.annotation_editor_on_metadata_update_done('region', pid.attr_id, update_count);
      }, function (err) {
  ...
  project_set_name(name: any): void {
    var _via_settings_project_name = name;

    var p = document.getElementById('project_name') as HTMLInputElement;
  ...
  project_save_confirmed(): void {
    ...
    let activeGroupObj = this.groupList.filter(realGroup => realGroup.name === this.activeGroup);
    var filename = this.activeGroup + '.json';
  ...
  project_add_new_file(filename: any, size: any, file_id: any): any {
    ...
    if (!this._via_img_metadata.hasOwnProperty(img_id)) {
      this._via_img_metadata[img_id] =  new file_metadata(filename, size);
      this._via_image_id_list.push(img_id);
  ...
  project_file_add_local(event: any): void {
    ...
    if (this._via_img_metadata) {
      ...
      this.show_message("Please upload some image files!", 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
    }
  }
  ...
  project_filepath_add(new_path: any): void {
    ...
    if (this._via_settings_core_filepath.hasOwnProperty(new_path)) {
      return;
    } else {
      var largest_order = 0;
      var path;
      for (path in this._via_settings_core_filepath) {
        if (this._via_settings_core_filepath[path] > largest_order) {
          largest_order = this._via_settings_core_filepath[path];
        }
      }
      this._via_settings_core_filepath[new_path] = largest_order + 1;

    }
  }

  project_filepath_del(path: any): void {
    if (this._via_settings_core_filepath.hasOwnProperty(path)) {
      delete this._via_settings_core_filepath[path];
    }
  }
  ...
  project_save_attributes(): void {
    ...
    this.save_data_to_local_file(all_region_data_blob, this._via_settings_project_name + '_attributes.json');
  }
  ...
  image_grid_set_content(img_index_list: any): void {
    ...
    var n = this._via_image_grid_img_index_list.length;
  ...
  image_grid_on_img_load(e: any): void {
    var img = e.target;
    var img_index = this.image_grid_parse_html_img_id(img.id);
  ...
  image_grid_add_img_if_possible(img: any): void {
    ...
    if (p.clientHeight < img_bottom_right_corner) {
      ...
      if (this._via_image_grid_stack_prev_page.length) {
        html.push('<span class="text_button" (click)="image_grid_page_prev()">Prev</span>');
      } else {
        html.push('<span>Prev</span>');
      }
      html.push('<span class="text_button" (click)="image_grid_page_next()">Next</span');
      ...
      var is_selected = (this._via_image_grid_selected_img_index_list.indexOf(img_index) !== -1);
      if (!is_selected) {
  ...
  image_grid_add_html_group_panel(d: any): void {
    ...
    del.setAttribute('(click)', 'image_grid_remove_group_by(this)');
    del.innerHTML = '&times;';
    p.appendChild(del);
    ...
    prev.setAttribute('(click)', 'image_grid_group_prev(this)');
    p.appendChild(prev);
    ...
    next.setAttribute('(click)', 'image_grid_group_next(this)');
    p.appendChild(next);
  ...
  image_grid_remove_group_by(p: any): void {
    ...
    if (group_index === 0) {
      ...
      var n = this._via_image_grid_group_var.length;
  ...
  image_grid_group_next(p: any): void {
    ...
    var current_index = this._via_image_grid_group_var[group_index].current_value_index;
    var next_index = current_index + 1;
    if (next_index >= n) {
      if (group_index === 0) {
  ...
  image_grid_page_nav_show_cancel(): void {
    ...
    html.push('<span class="text_button" (click)="image_grid_cancel_load_ongoing()">Cancel</span>');
    info.innerHTML = html.join('');
  }
  ...
  settings_save(): void {
    ...
    if (p.nodeValue !== this._via_settings_project_name) {
      p.nodeValue = this._via_settings_project_name;
    }
  ...
  settings_show_current_value(): void {
    ...
    for (i = 0; i < n; ++i) {
      var s = vl[i].childNodes[1];
      var sid_parts = s.nodeValue.split('.');
      if (sid_parts[0] === '_via_settings') {
        var el = this._via_settings;
        var found = true;
        var j;
  ...
  settings_region_visualisation_update_options(): void {
    ...
    for (var setting in region_setting_list) {
      var select = document.getElementById('_via_settings_ui_image_' + setting);
      select.innerHTML = '';
      var default_option = document.createElement('option');
      default_option.setAttribute('value', region_setting_list[setting]['default_option']);
      if (this._via_settings_ui_image[setting] === region_setting_list[setting]['default_option']) {
  ...
  settings_filepath_update_html(): void {
    var p = document.getElementById('_via_settings_core_filepath');
    p.innerHTML = '';
    var i, path, order;
    for (path in this._via_settings_core_filepath) {
      order = this._via_settings_core_filepath[path]
      if (order !== 0) {
        var li = document.createElement('li');
        li.innerHTML = path + '<span class="text_button" title="Delete image path" (click)="project_filepath_del(\"' + path + '\"); settings_filepath_update_html();">&times;</span>';
        p.appendChild(li);
      }
    }
  }
  ...
  polygon_to_bbox(pts: any): number[] {
	...
	_via_reg_canvas_mousemove_handler(e: MouseEvent): void {
		if (e.type === 'mousemove') {
      if (!this._via_current_image_loaded) {
        console.log('Image not loaded yet')
			...
			var rf = this.region_info.nativeElement;
			if (rf != null && this._via_is_region_info_visible) {
				var img_x = Math.round(this._via_current_x * this._via_canvas_scale);
				var img_y = Math.round(this._via_current_y * this._via_canvas_scale);
				rf.innerHTML = 'X:' + img_x + ',' + ' Y:' + img_y;
			}

			if (this._via_is_region_selected) {
				// display the region's info if a region is selected
				if (rf != null && this._via_is_region_info_visible && this._via_user_sel_region_id !== -1) {
					...
					switch (canvas_attr['name']) {
						...
						case this.VIA_REGION_SHAPE.CIRCLE:
							 rf = this.region_info.nativeElement;
				...
				if (!this._via_is_user_resizing_region) {
					...
					var _via_region_edge = this.is_on_region_corner(this._via_current_x, this._via_current_y);
					...
					if (_via_region_edge[0] === this._via_user_sel_region_id) {
						switch (_via_region_edge[1]) {
							...
							case 7: // bottom-middle point of rect
								this._via_reg_canvas.style.cursor = "ns-resize";
								break;
							case 6: // Fall-through // top-middle point of rect
							case 8: // bottom-middle point of rect
								this._via_reg_canvas.style.cursor = "ew-resize";
								break;
						...
						if (_via_region_edge[1] >= this.VIA_POLYGON_RESIZE_VERTEX_OFFSET) {
						...
						var yes = this.is_inside_this_region(this._via_current_x,
							this._via_current_y,
          ...
          this._via_reg_ctx.clearRect(0, 0, this._via_reg_canvas.width, this._via_reg_canvas.height);
				...
				var region_x0 = this._via_click_x0;
				var region_y0 = this._via_click_y0;
				...
				switch (this._via_current_shape) {
					...
					case this.VIA_REGION_SHAPE.ELLIPSE:
						this._via_draw_ellipse_region(region_x0, region_y0, dx, dy, 0, false);
          ...
          console.log('clearing rectangle');
				...
				var region_id = this._via_region_edge[0];
				var attr = this._via_canvas_regions[region_id].shape_attributes;
				switch (attr['name']) {
					case this.VIA_REGION_SHAPE.RECT:
						// original rectangle
						...
						var mx = this._via_current_x;
						var my = this._via_current_y;
						var preserve_aspect_ratio = false;
						// constrain (mx,my) to lie on a line connecting a diagonal of rectangle
						if (this._via_is_ctrl_pressed) {
							preserve_aspect_ratio = true;
						}

						this.rect_update_corner(this._via_region_edge[1], d, mx, my, preserve_aspect_ratio);
					...
					case this.VIA_REGION_SHAPE.CIRCLE:
						var dx = Math.abs(attr['cx'] - this._via_current_x);
						var dy = Math.abs(attr['cy'] - this._via_current_y);
					...
					case this.VIA_REGION_SHAPE.ELLIPSE:
						...
						switch (_via_region_edge[1]) {
							case 5:
								...
								new_theta = Math.atan2(- (this._via_current_x - attr['cx']), (this._via_current_y - attr['cy']));
								break;
	        ...
	        this._via_reg_ctx.clearRect(0, 0, this._via_reg_canvas.width, this._via_reg_canvas.height);
				...
				var move_x = (this._via_current_x - this._via_region_click_x);
				var move_y = (this._via_current_y - this._via_region_click_y);
				var attr = this._via_canvas_regions[this._via_user_sel_region_id].shape_attributes;
			...
			if (this._via_is_user_drawing_polygon) {
				this._via_redraw_reg_canvas();
				var attr = this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes;
				var all_points_x = attr['all_points_x'];
				var all_points_y = attr['all_points_y'];
				var npts = all_points_x.length;

				if (npts > 0) {
					var line_x = [all_points_x.slice(npts - 1), this._via_current_x];
					var line_y = [all_points_y.slice(npts - 1), this._via_current_y];
...
//    this.attribute_update_panel_set_active_button(); // This is tentative and I will only hold on to it to test if it affects it significantly.
//    this.annotation_editor_set_active_button(); // This is tentative and I will only hold on to it to test if it affects it significantly.
  ...
  _via_init_reg_canvas_context() {    
    this._via_reg_ctx = this.ctx
    var img_id;
		var current_image = new Image()
    current_image.src = this.getPath();
    this._via_current_image_width = current_image.width
    this._via_current_image_height =  current_image.height
//    this._via_reg_ctx.style.backgroundImage = this.getPath()
//    this.ctx.drawImage(current_image, 0, 0, current_image.width, current_image.height);
    console.log('Image drawn successfully', current_image.width, current_image.height);
    this._via_current_image_loaded = true;
    var n = this.getActiveFeatures().length
    var asset; 
    for (var asset of this.getActiveFeatures()){  
      console.log(this.getAssetDisplay(asset)) 
      this._via_image_id_list.push(asset)
    }

    this._via_image_id = this._via_image_id_list[0]
    img_id = this._via_image_id_list[0]
    this._via_img_metadata[img_id] = new file_metadata(img_id, -1)
    this._via_image_filename_list.push(img_id)
    this._via_img_count += 1;
  ...
  _via_init_keyboard_handlers() {
    // window.addEventListener('keydown', this._via_window_keydown_handler, false); // Replaced with @HostListener!
  ...
  // handles drawing of regions over image by the user
	_via_init_mouse_handlers(e: MouseEvent): void {
		if (e.type === 'dblclick') {
			this._via_reg_canvas_dblclick_handler(e);
			return;		}
  }
  ...
  download_as_image() {
    if (this._via_display_area_content_name !== VIA_DISPLAY_AREA_CONTENT_NAME['IMAGE']) {
      ...
      // draw current image
      ct.drawImage(this._via_current_image, 0, 0, this._via_reg_canvas.width, this._via_reg_canvas.height);
  ...
  map_to_json(m: any): string {
    ...
    for (var key in m) {
      ...
      si += this.VIA_CSV_KEYVAL_SEP;
      si += JSON.stringify(v);
      s.push(si);
    }
    return '{' + s.join(this.VIA_CSV_SEP) + '}';
  }
 ...
 ///REWWRITE ALL THIS AS A CASE STATEMENT (PACK_VIA_METADATA)
  pack_via_metadata(return_type: any): Promise<unknown> {
    return new Promise( function(ok_callback, err_callback) {
        ...
        for ( var image_id in this._via_img_metadata ) {
          ...
          if ( r.length !==0 ) {
            for ( var i = 0; i < r.length; ++i ) {
              ...
              var sattr = this.map_to_json( r[i].shape_attributes );
              sattr = '"' +  this.escape_for_csv( sattr ) + '"';
              csvline.push(sattr);
  
              var rattr = this.map_to_json( r[i].region_attributes );
              rattr = '"' +  this.escape_for_csv( rattr ) + '"';
              csvline.push(rattr);
              csvdata.push( csvline.join(this.VIA_CSV_SEP) );
            }
          } else {
      ...
      if( return_type === 'coco' ) {
        this.img_stat_set_all().then( function(ok) {
  ...
  _via_regions_group_color_init(): void {
    ...
    var aid = this._via_settings_ui_image_region_color;
  ...
  _via_load_canvas_regions(): void {
    ...
    for (var i = 0; i < regions.length; ++i) {
      ...
      switch (this._via_canvas_regions[i].shape_attributes['name']) {
        case this.VIA_REGION_SHAPE.RECT:
          var x = regions[i].shape_attributes['x'] / this._via_canvas_scale;
          var y = regions[i].shape_attributes['y'] / this._via_canvas_scale;
          var width = regions[i].shape_attributes['width'] / this._via_canvas_scale;
          var height = regions[i].shape_attributes['height'] / this._via_canvas_scale;
        ...
        case this.VIA_REGION_SHAPE.CIRCLE:
          var cx = regions[i].shape_attributes['cx'] / this._via_canvas_scale;
          var cy = regions[i].shape_attributes['cy'] / this._via_canvas_scale;
          var r = regions[i].shape_attributes['r'] / this._via_canvas_scale;
        ...
        case this.VIA_REGION_SHAPE.ELLIPSE:
          cx = regions[i].shape_attributes['cx'] / this._via_canvas_scale;
          cy = regions[i].shape_attributes['cy'] / this._via_canvas_scale;
          var rx = regions[i].shape_attributes['rx'] / this._via_canvas_scale;
          var ry = regions[i].shape_attributes['ry'] / this._via_canvas_scale;
          // rotation in radians
        ...
        case this.VIA_REGION_SHAPE.POLYGON:
          ...
          for (var j = 0; j < all_points_x.length; ++j) {
            all_points_x[j] = Math.round(all_points_x[j] / this._via_canvas_scale);
            all_points_y[j] = Math.round(all_points_y[j] / this._via_canvas_scale);
          }
  ...
  select_region_shape(sel_shape_name: any): void {
    for (var shape_name in this.VIA_REGION_SHAPE) {
      var ui_element = document.getElementById('region_shape_' + this.VIA_REGION_SHAPE[shape_name]);
      ui_element.classList.remove('selected');
    }

    this._via_current_shape = sel_shape_name;
    var ui_element = document.getElementById('region_shape_' + this._via_current_shape);
    ...
    switch (this._via_current_shape) {
      ...
      case 'ellipse':
        this.show_message('Press single click and drag mouse to draw ' +
          this._via_current_shape + ' region', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
        break;
      ...
      case 'polygon':
        ...
        this.show_message('[Single Click] to define polygon/polyline vertices, ' +
          '[Backspace] to delete last vertex, [Enter] to finish, [Esc] to cancel drawing.', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
        break;

      case 'point':
        this.show_message('Press single click to define points (or landmarks)', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
        break;

      default:
        this.show_message('Unknown shape selected!', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
        break;
    }
  }
  ...
  toggle_all_regions_selection(is_selected: any): void {
    ...
    var i;
    this._via_region_selected_flag = [];
    for (i = 0; i < n; ++i) {
      this._via_region_selected_flag[i] = is_selected;
    }
  ...
  // user clicks on the canvas
	_via_reg_canvas_mousedown_handler(e: MouseEvent): void {
    if (e.type == 'mousedown') {
      console.log('mousedown status acheived')
    //  this.mmouseedown.unsubscribe()
    //  this.propagateNot = true;
    this._via_click_x0 = e.offsetX; this._via_click_y0 = e.offsetY;
    this._via_region_edge = this.is_on_region_corner(this._via_click_x0, this._via_click_y0);
    this.region_id = this.is_inside_region(this._via_click_x0, this._via_click_y0, true);
    ...
    if (this._via_is_region_selected) {
      ...
      if (this._via_region_edge[1] > 0) {
        ...
        var yes = this.is_inside_this_region(this._via_click_x0,
          this._via_click_y0,
          this._via_user_sel_region_id);
        if (yes) {
          if (!this._via_is_user_moving_region) {
      ...
      if (this.region_id === -1) {
        ...
        if (this._via_current_shape !== this.VIA_REGION_SHAPE.POLYGON &&
          this._via_current_shape !== this.VIA_REGION_SHAPE.POLYLINE &&
          this._via_current_shape !== this.VIA_REGION_SHAPE.POINT) {
    ...
    // this.Mmousedown.subscribe((e: MouseEvent) => {
    //   console.log('mousedown_fired');
    //   this._via_reg_canvas_mousedown_handler(e)
    // }) 
	};
  ...
  //  - new region drawing (including polygon)
	_via_reg_canvas_mouseup_handler(e: MouseEvent): void {
		if (e.type === 'mouseup') {
      this.propagateNot = true;
			this._via_click_x1 = e.offsetX; this._via_click_y1 = e.offsetY;

			var click_dx = Math.abs(this._via_click_x1 - this._via_click_x0);
			var click_dy = Math.abs(this._via_click_y1 - this._via_click_y0);
			...
			if (this._via_is_user_moving_region) {
				...
				var move_x = Math.round(this._via_click_x1 - this._via_region_click_x);
				var move_y = Math.round(this._via_click_y1 - this._via_region_click_y);

				if (Math.abs(move_x) > this.VIA_MOUSE_CLICK_TOL ||
					Math.abs(move_y) > this.VIA_MOUSE_CLICK_TOL) {
					// move all selected regions
					this._via_move_selected_regions(move_x, move_y);
				} else {
					...
					// traverse the canvas regions in alternating ascending
					// and descending order to solve the issue of nested regions
					var nested_region_id = this.is_inside_region(this._via_click_x0, this._via_click_y0, true);
					if (nested_region_id >= 0 &&
						...
						//!!!!!!!!!!!!!!!!!!! Find a way to handle the keyboard(if) event, with shiftkey commented out here. 
						if (!(e).shiftKey) {
							this.toggle_all_regions_selection(false);
						}
						...
						switch (this._via_current_shape) {
							case this.VIA_REGION_SHAPE.POLYLINE: // handled by case for POLYGON
							case this.VIA_REGION_SHAPE.POLYGON:
								...
								canvas_polygon_region.shape_attributes['name'] = this._via_current_shape;
								canvas_polygon_region.shape_attributes['all_points_x'] = [Math.round(this._via_click_x0)];
								canvas_polygon_region.shape_attributes['all_points_y'] = [Math.round(this._via_click_y0)];
								var new_length = this._via_canvas_regions.push(canvas_polygon_region);
								this._via_current_polygon_region_id = new_length - 1;
								break;

							case this.VIA_REGION_SHAPE.POINT:
								// user has marked a landmark point
								var point_region = new file_region();
								point_region.shape_attributes['name'] = this.VIA_REGION_SHAPE.POINT;
								point_region.shape_attributes['cx'] = Math.round(this._via_click_x0 * this._via_canvas_scale);
								point_region.shape_attributes['cy'] = Math.round(this._via_click_y0 * this._via_canvas_scale);
								...
								canvas_point_region.shape_attributes['cx'] = Math.round(this._via_click_x0);
								canvas_point_region.shape_attributes['cy'] = Math.round(this._via_click_y0);
			...
			if (this._via_is_user_resizing_region) {
				...
				switch (canvas_attr['name']) {
					case this.VIA_REGION_SHAPE.RECT:
						...
						image_attr['x'] = Math.round(d[0] * this._via_canvas_scale);
						image_attr['y'] = Math.round(d[1] * this._via_canvas_scale);
						image_attr['width'] = Math.round(w * this._via_canvas_scale);
						image_attr['height'] = Math.round(h * this._via_canvas_scale);

						canvas_attr['x'] = Math.round(image_attr['x'] / this._via_canvas_scale);
						canvas_attr['y'] = Math.round(image_attr['y'] / this._via_canvas_scale);
						canvas_attr['width'] = Math.round(image_attr['width'] / this._via_canvas_scale);
						canvas_attr['height'] = Math.round(image_attr['height'] / this._via_canvas_scale);
						break;

					case this.VIA_REGION_SHAPE.CIRCLE:
						var dx = Math.abs(canvas_attr['cx'] - this._via_current_x);
						var dy = Math.abs(canvas_attr['cy'] - this._via_current_y);
						var new_r = Math.sqrt(dx * dx + dy * dy);

						image_attr['r'] = fixfloat(new_r * this._via_canvas_scale);
						canvas_attr['r'] = Math.round(image_attr['r'] / this._via_canvas_scale);
						break;
					...
					case this.VIA_REGION_SHAPE.ELLIPSE:
						...
						switch (this._via_region_edge[1]) {
							case 5:
								new_ry = Math.sqrt(dx * dx + dy * dy);
								new_theta = Math.atan2(- (this._via_current_x - canvas_attr['cx']), (this._via_current_y - canvas_attr['cy']));
								break;

							case 6:
								new_rx = Math.sqrt(dx * dx + dy * dy);
								new_theta = Math.atan2((this._via_current_y - canvas_attr['cy']), (this._via_current_x - canvas_attr['cx']));
								break;
						...
						image_attr['rx'] = fixfloat(new_rx * this._via_canvas_scale);
						image_attr['ry'] = fixfloat(new_ry * this._via_canvas_scale);
						image_attr['theta'] = fixfloat(new_theta);

						canvas_attr['rx'] = Math.round(image_attr['rx'] / this._via_canvas_scale);
						canvas_attr['ry'] = Math.round(image_attr['ry'] / this._via_canvas_scale);
						canvas_attr['theta'] = fixfloat(new_theta);
						break;

					case VIA_REGION_SHAPE.POLYLINE: // handled by polygon
					case VIA_REGION_SHAPE.POLYGON:
						var moved_vertex_id = this._via_region_edge[1] - this.VIA_POLYGON_RESIZE_VERTEX_OFFSET;


						// Another if statement regarding KeyBoard Event handler. Look at it later. 
            if ((e).ctrlKey) {
              ...
              var shape = r
              var is_on_vertex = this.is_on_polygon_vertex(r['all_points_x'], r['all_points_y'], this._via_current_x, this._via_current_y);

              if (is_on_vertex === this._via_region_edge[1]) {
                // click on vertex, hence delete vertex
                if (this._via_polygon_del_vertex(region_id, moved_vertex_id)) {
                  this.show_message('Deleted vertex ' + moved_vertex_id + ' from region', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
                }
              } else {
                var is_on_edge = is_on_polygon_edge(r['all_points_x'], r['all_points_y'], this._via_current_x, this._via_current_y);
                if (is_on_edge === this._via_region_edge[1]) {
                  // click on edge, hence add new vertex
                  var vertex_index = is_on_edge - this.VIA_POLYGON_RESIZE_VERTEX_OFFSET;
                  var canvas_x0 = Math.round(this._via_click_x1);
                  var canvas_y0 = Math.round(this._via_click_y1);
                  var img_x0 = Math.round(canvas_x0 * this._via_canvas_scale);
                  var img_y0 = Math.round(canvas_y0 * this._via_canvas_scale);
                  canvas_x0 = Math.round(img_x0 / this._via_canvas_scale);
                  canvas_y0 = Math.round(img_y0 / this._via_canvas_scale);
              ...
              var imx = Math.round(this._via_current_x * this._via_canvas_scale);
              var imy = Math.round(this._via_current_y * this._via_canvas_scale);
              image_attr['all_points_x'][moved_vertex_id] = imx;
              image_attr['all_points_y'][moved_vertex_id] = imy;
              canvas_attr['all_points_x'][moved_vertex_id] = Math.round(imx / this._via_canvas_scale);
              canvas_attr['all_points_y'][moved_vertex_id] = Math.round(imy / this._via_canvas_scale);
			...
			if (click_dx < this.VIA_MOUSE_CLICK_TOL ||
				click_dy < this.VIA_MOUSE_CLICK_TOL) {
				// if user is already drawing polygon, then each click adds a new point
				if (this._via_is_user_drawing_polygon) {
					var canvas_x0 = Math.round(this._via_click_x1);
					var canvas_y0 = Math.round(this._via_click_y1);
					var n = this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_x'].length;
					var last_x0 = this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_x'][n - 1];
					var last_y0 = this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_y'][n - 1];
					// discard if the click was on the last vertex
					if (canvas_x0 !== last_x0 || canvas_y0 !== last_y0) {
						// user clicked on a new polygon point
						this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_x'].push(canvas_x0);
						this._via_canvas_regions[this._via_current_polygon_region_id].shape_attributes['all_points_y'].push(canvas_y0);
					}
				} else {
            ...
            if (!(e).shiftKey) {
              ...
              this.annotation_editor_hide();
						...
						if (this._via_is_region_info_visible) {
							var canvas_attr = this._via_canvas_regions[region_id].shape_attributes;
						...
						this.show_message('Region selected. If you intended to draw a region, click again inside the selected region to start drawing a region.', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS)
					} else {
						if (this._via_is_user_drawing_region) {
			...
			if (this._via_is_user_drawing_region) {
				...
				var region_x0 = this._via_click_x0;
				var region_y0 = this._via_click_y0;
				var region_x1 = this._via_click_x1;
				var region_y1 = this._via_click_y1;
				...
				if (region_dx > this.VIA_REGION_MIN_DIM && region_dy > this.VIA_REGION_MIN_DIM) { // avoid regions with 0 dim
					switch (this._via_current_shape) {
						case this.VIA_REGION_SHAPE.RECT:
							...
							var x = Math.round(region_x0 * this._via_canvas_scale);
							var y = Math.round(region_y0 * this._via_canvas_scale);
							var width = Math.round(region_dx * this._via_canvas_scale);
							var height = Math.round(region_dy * this._via_canvas_scale);
							...
							canvas_img_region.shape_attributes['x'] = Math.round(x / this._via_canvas_scale);
							canvas_img_region.shape_attributes['y'] = Math.round(y / this._via_canvas_scale);
							canvas_img_region.shape_attributes['width'] = Math.round(width / this._via_canvas_scale);
							canvas_img_region.shape_attributes['height'] = Math.round(height / this._via_canvas_scale);

							new_region_added = true;
							break;

						case this.VIA_REGION_SHAPE.CIRCLE:
							var cx = Math.round(region_x0 * this._via_canvas_scale);
							var cy = Math.round(region_y0 * this._via_canvas_scale);
							let r = Math.round(Math.sqrt(region_dx * region_dx + region_dy * region_dy) * this._via_canvas_scale);
							...
							canvas_img_region.shape_attributes['cx'] = Math.round(cx / this._via_canvas_scale);
							canvas_img_region.shape_attributes['cy'] = Math.round(cy / this._via_canvas_scale);
							canvas_img_region.shape_attributes['r'] = Math.round(r / this._via_canvas_scale);

							new_region_added = true;
							break;

						case this.VIA_REGION_SHAPE.ELLIPSE:
							cx = Math.round(region_x0 * this._via_canvas_scale);
							cy = Math.round(region_y0 * this._via_canvas_scale);
							var rx = Math.round(region_dx * this._via_canvas_scale);
							var ry = Math.round(region_dy * this._via_canvas_scale);
							var theta = 0;
							...
							canvas_img_region.shape_attributes['cx'] = Math.round(cx / this._via_canvas_scale);
							canvas_img_region.shape_attributes['cy'] = Math.round(cy / this._via_canvas_scale);
							canvas_img_region.shape_attributes['rx'] = Math.round(rx / this._via_canvas_scale);
							canvas_img_region.shape_attributes['ry'] = Math.round(ry / this._via_canvas_scale);
							canvas_img_region.shape_attributes['theta'] = theta;

							new_region_added = true;
							break;

						case this.VIA_REGION_SHAPE.POINT:    // handled by case VIA_REGION_SHAPE.POLYGON
						case this.VIA_REGION_SHAPE.POLYLINE: // handled by case VIA_REGION_SHAPE.POLYGON
						case this.VIA_REGION_SHAPE.POLYGON:
					...
					if (new_region_added) {
						...
						if (n1 !== n2) {
							console.log('this._via_img_metadata.regions[' + n1 + '] and this._via_canvas_regions[' + n2 + '] count mismatch');
						}
                ...
                this.annotation_editor_add_row(new_region_id);
					...
					this.show_message('Prevented accidental addition of a very small region.', 2 * this.VIA_THEME_MESSAGE_TIMEOUT_MS);
				}
				return;
			}
		}
  ...
  ngOnDestroy() {
    ...
    this.mmousemove.unsubscribe(); // Make the obserables (typeof) subscription and unsuscribe
    this.mmouseedown.unsubscribe();
    this.mmouseover.unsubscribe();
    this.mmouseup.unsubscribe();
    this.wheelevent.unsubscribe();
    this.ddblclick.unsubscribe();
    this.kkeyup.unsubscribe();
    this.kkeydown.unsubscribe();
